...
def create_app(test_config=None):
  ...
  def get_questions_based_on_category(category_id):
    try:
      category_type = Category.query.filter_by(id = category_id).one_or_none()

      if category_type is None:
        abort(400, 'Invalid category id')

      selection = Question.query.filter(Question.category == str(category_type.id)).all()
